...
function parsePermissions(input: string) {
  ...
  const officialKeys = Object.keys(PermissionFlagsBits) as Array<
...
function buildOverwriteOptions(allowBitfield: bigint, denyBitfield: bigint) {
  ...
  > = {};
  const keys = Object.keys(PermissionFlagsBits) as Array<
    keyof typeof PermissionFlagsBits
  >;
...
function overwritesEqual(a: any, b: any): boolean {
  ...
  for (const owA of aArr) {
    ...
    const allowA =
      typeof owA.allow?.bitfield !== "undefined"
        ? BigInt(owA.allow.bitfield)
        : BigInt(owA.allow ?? 0);
    const denyA =
      typeof owA.deny?.bitfield !== "undefined"
        ? BigInt(owA.deny.bitfield)
        : BigInt(owA.deny ?? 0);

    const allowB =
      typeof owB.allow?.bitfield !== "undefined"
        ? BigInt(owB.allow.bitfield)
        : BigInt(owB.allow ?? 0);
    const denyB =
      typeof owB.deny?.bitfield !== "undefined"
        ? BigInt(owB.deny.bitfield)
        : BigInt(owB.deny ?? 0);
...
function isSyncedToParent(ch: EditableChannel): boolean {
  ...
  const childOverwrites =
    (ch as any).permissionOverwrites?.cache ?? (ch as any).permissionOverwrites;
  const parentOverwrites =
    (parent as any).permissionOverwrites?.cache ??
    (parent as any).permissionOverwrites;
...
export default {
  data: new SlashCommandBuilder()
    .setName("permissions")
    .setDescription(
      "Manage role permissions across channels and categories (uses embeds)."
    )
    .addSubcommand((sc) =>
      sc
        .setName("apply")
        .setDescription(
          "Apply allow/deny permission changes to selected channels/categories."
        )
        .addRoleOption((opt) =>
        ...
        .addStringOption((opt) =>
          opt
            .setName("allowperms")
            .setDescription("Comma-separated permissions to ALLOW")
            .setRequired(false)
        )
        .addStringOption((opt) =>
          opt
            .setName("denyperms")
            .setDescription("Comma-separated permissions to DENY")
            .setRequired(false)
        )
        .addStringOption((opt) =>
          opt
            .setName("includechannels")
            .setDescription(
              "Comma-separated channel IDs to explicitly include (overrides category selection)"
            )
            .setRequired(false)
        )
        .addStringOption((opt) =>
          opt
            .setName("includecategories")
            .setDescription(
              "Comma-separated category IDs: only modify channels under these (optional)"
            )
            .setRequired(false)
        )
        .addStringOption((opt) =>
          opt
            .setName("excludecategories")
            .setDescription(
              "Comma-separated category IDs to exclude channels under them from modification (optional)"
            )
            .setRequired(false)
        )
        .addBooleanOption((opt) =>
          opt
            .setName("modifycategories")
            .setDescription(
              "Also modify the category objects themselves if their IDs are targeted or matched?"
            )
            .setRequired(false)
        )
        .addStringOption((opt) =>
          opt
            .setName("preset")
            .setDescription(
              "Name of a saved preset to apply (overridden by explicit allow/deny inputs)"
            )
            .setRequired(false)
        )
    )
    .addSubcommand((sc) =>
      sc
        .setName("save")
        .setDescription(
          "Save a permissions preset for this guild (name + allow/deny strings)."
        )
        .addStringOption((opt) =>
    ...
    .addSubcommand((sc) =>
      sc.setName("list").setDescription("List saved presets for this guild.")
    )
    .addSubcommand((sc) =>
      sc
        .setName("delete")
        ...
        .addStringOption((opt) =>
          opt
            .setName("name")
            .setDescription("Preset name to delete")
            .setRequired(true)
        )
    ),
    ...
    try {
      if (sub === "save") {
        ...
        await interaction.editReply({
          embeds: [
            successEmbed(
              "Preset saved",
              `Saved preset **${name}** for this server.`
            ),
          ],
        });
        return;
      ...
      if (sub === "list") {
        ...
        if (names.length === 0) {
          await interaction.editReply({
            embeds: [
              infoEmbed("No presets", "No presets saved for this server."),
            ],
          });
          return;
        }
        ...
        for (const n of names) {
          ...
          embed.addFields([
            {
              name: n,
              value: `Allow: ${allow || "(none)"}\nDeny: ${
                deny || "(none)"
              }\nSaved: ${p.savedAt ?? "unknown"}`,
              inline: false,
            },
          ]);
        }
        ...
        if (!ok) {
          await interaction.editReply({
            embeds: [
              errorEmbed(
                "Preset not found",
                `Preset **${name}** not found for this server.`
              ),
            ],
          });
        } else {
          await interaction.editReply({
            embeds: [
              successEmbed("Preset deleted", `Deleted preset **${name}**.`),
            ],
          });
        }
        return;
      ...
      if (sub === "apply") {
        ...
        const includeChannelsInput =
          interaction.options.getString("includechannels") ?? "";
        const includeCategoriesInput =
          interaction.options.getString("includecategories") ?? "";
        const excludeCategoriesInput =
          interaction.options.getString("excludecategories") ?? "";
        const modifyCategories =
          interaction.options.getBoolean("modifycategories") ?? false;
          ...
          if (!presets[presetName]) {
            await interaction.editReply({
              embeds: [
                errorEmbed(
                  "Preset not found",
                  `Preset **${presetName}** not found for this server.`
                ),
              ],
            });
            return;
        ...
        const { flags: allowFlagsRaw, invalid: invalidAllow } =
          parsePermissions(allowInput);
        const { flags: denyFlagsRaw, invalid: invalidDeny } =
          parsePermissions(denyInput);
          ...
          await interaction.editReply({
            embeds: [
              errorEmbed(
                ...
                `Invalid: ${invalidAll.join(
                  ", "
                )}. Use PermissionFlagsBits keys (e.g. ViewChannel) or common synonyms.`
              ),
            ],
          });
          return;
        ...
        const allowBitfield = finalAllow.length
          ? PermissionsBitField.resolve(finalAllow)
          : 0n;
        const denyBitfield = finalDeny.length
          ? PermissionsBitField.resolve(finalDeny)
          : 0n;

        const explicitChannelIds = includeChannelsInput
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean);
        const includeCategoryIds = includeCategoriesInput
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean);
        const excludeCategoryIds = excludeCategoriesInput
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean);
          ...
          for (const id of explicitChannelIds) {
            ...
            if (parent) {
              ...
              if (
                ...
                includeCategoryIds.includes(parent.id)
              ) {
                continue;
              }
            }
            ...
            if ((ch as any).type === ChannelType.GuildCategory) {
              if (modifyCategories) {
                if (includeCategoryIds.length > 0) {
                  if (includeCategoryIds.includes(ch.id))
                    categoriesToEdit.set(ch.id, ch as CategoryChannel);
                } else {
                  categoriesToEdit.set(ch.id, ch as CategoryChannel);
                }
        ...
        const uniqueChannels = Array.from(
          new Map(finalChannels.map((c) => [c.id, c])).values()
        );
        ...
        if (categoriesToEdit.size === 0 && uniqueChannels.length === 0) {
          await interaction.editReply({
            embeds: [
              infoEmbed(
                "Nothing to do",
                "No editable categories/channels found with the provided parameters."
              ),
            ],
          });
          return;
        ...
        const overwriteOptions = buildOverwriteOptions(
          allowBitfield,
          denyBitfield
        );
        ...
        for (const [id, cat] of categoriesToEdit) {
          try {
            await (cat as any).permissionOverwrites.edit(
              role,
              overwriteOptions
            );
            successCount++;
        ...
        const summaryEmbed = new EmbedBuilder()
          ...
          .addFields(
            ...
            {
              name: "Categories processed",
              value: `${categoriesToEdit.size}`,
              inline: true,
            },
            {
      ...
      await interaction.editReply({
        embeds: [errorEmbed("Command error", String(err))],
      });
    }
  },
};
