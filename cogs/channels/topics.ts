...
export default {
      ...
      subcommand
        ...
        .addChannelOption((option) =>
          option
            .setName("channel")
            .setDescription(
              "Channel to set topic for (defaults to current channel)"
            )
            .setRequired(false)
      ...
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName("channels")
            .setDescription(
              "Comma-separated list of channels to include (leave empty for all channels)"
            )
            .setRequired(false)
            .setAutocomplete(true)
        )
  ...
  async autocomplete(interaction: AutocompleteInteraction) {
    ...
    if (focusedOption.name === "category") {
      ...
      focusedOption.name === "channels" ||
      focusedOption.name === "exclude"
    ) {
      ...
      const filtered = availableChannels
        .filter((channel) =>
          channel.toLowerCase().includes(currentSearch.toLowerCase())
        )
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === "set") {
      ...
      const skipExisting =
        interaction.options.getBoolean("skip_existing") ?? false;
      ...
      const validChannels = Array.from(channels.values()).filter((channel) => {
        ...
        if (
          category &&
          channel.parent?.name.toLowerCase() !== category.toLowerCase()
        ) {
          return false;
        }
        ...
        if (
          targetChannelNames &&
          !targetChannelNames.includes(channel.name.toLowerCase())
        ) {
          return false;
        }
