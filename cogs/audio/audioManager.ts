...
export async function attachLavalink(client: Client) {
  ...
  client.on("raw", (packet) => {
    ...
    if (!t) return;
    if (
      ["VOICE_STATE_UPDATE", "VOICE_SERVER_UPDATE", "CHANNEL_DELETE"].includes(
        t
      )
    ) {
  ...
  lavalink.on(
    ...
    (player: Player, track: Track | null, _payload: TrackStartEvent) => {
      ...
      console.log(
        `[Lavalink] Track started on guild ${player.guildId}: ${track.info.title}`
      );
  ...
  if (client.user) {
    ...
    client.once("ready", () => {
      ...
      console.log(
        "[Lavalink] Initializing with bot user:",
...
export async function getAvailableNode(): Promise<LavalinkNode> {
  ...
  if (!nodes || nodes.length === 0) {
    console.log(
      "[Lavalink] No nodes available, waiting for a node to connect..."
    );
